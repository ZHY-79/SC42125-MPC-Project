...
# def find_lookahead_point(pos, reference_trajectory, lookahead_distance):
...
#         # 如果点在前方且距离大于等于前视距离，则选择该点
#         if is_in_front and dist >= lookahead_distance:
#             return i
    
#     # 如果没有找到满足条件的点，返回轨迹的最后一个点
#     return len(reference_trajectory) - 1
...
def mpc_controller(pos, reference_traj, sampled_traj, N, dt, L, lookahead_distance=2.0, initial_state=None, v_ref=2, sigma_ref=0):      # 注意这里vref = 0， 因为要停下来，但是无法求解
    ...
    # 定义状态和控制权重
    Q = np.diag([1000, 1000, 1000])                                               # 状态误差权重矩阵 [x, y, theta]
    R = np.diag([2, 2])                                                   # 控制偏差权重矩阵 [v, sigma]
    ...
    for _ in range(N):
        lbx.extend([-0.4])                                                     # 最小转向角偏差
        ubx.extend([0.4])                                                      # 最大转向角偏差
