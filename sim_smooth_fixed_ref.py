...
def dynamicUpdate(MatrixA, MatrixB, error_k, inputerror_k):
    ...
    # Calculate next state error using discrete dynamic equation
...
def find_lookahead_point(pos, sampled_trajectory, lookahead_distance):
    ...
    # Calculate unit vector of vehicle heading
    heading_vector = np.array([np.cos(current_heading), np.sin(current_heading)])
    
    # Find the closest point that satisfies the lookahead distance ahead
    min_dist = float('inf')
    ...
    for i, point in enumerate(sampled_trajectory):
        ...
        # Positive dot product means reference point is ahead of vehicle
        is_in_front = np.dot(heading_vector, point_vector) > 0
        
        # If point is ahead and distance is greater than or equal to lookahead distance, check if it's closer
...
def sample_trajectory(original_trajectory, num_segments=100):
    ...
    # First sampling point starts from segment_size, not from 0 (skip start point)
    ...
    # Return actual trajectory points, not indices
...
def mpc_controller(pos, reference_traj, sampled_traj, N, dt, L, lookahead_distance=2.0, initial_state=None, v_ref=2, sigma_ref=0):
    ...
    R_gain = 3
    Q = Q * Q_gain
    R = R * R_gain                                                   # Control deviation weight matrix [v, sigma]
...
def run_prius_with_controller(csv_file="bicycle_constrained_trajectory.csv", 
    ...
    ax1.legend()
    first_point = ax1.plot(sampled_traj[0][0], sampled_traj[0][1], 'x', color='red', label='Reference Points')
    for ref in sampled_traj:
        ax1.plot(ref[0], ref[1], 'x', color='red')
    ax1.legend()
    ...
    with tqdm(total=n_steps, desc="Progress") as progress_bar:
        for i in range(n_steps):
            ...
            # Use MPC to calculate control inputs and get optimal cost and stage cost
            velocity, steering_angle, optimal_cost, stage_cost, velocity_error, steer_error, P = mpc_controller(pos, trajectory, sampled_traj, N, dt, 4.6, lookahead_distance=lookahead_distance, v_ref=v_ref, sigma_ref=0)
            ...
            # Calculate error - find closest reference point
...
def plot_inputs_and_errors(time_points, velocity_inputs, steering_inputs, 
    ...
    plt.tight_layout()
    fig2.savefig("adaptive_imgs/mpc_control_inputs.png", bbox_inches='tight')
    ...
    plt.tight_layout()
    fig3.savefig("adaptive_imgs/mpc_tracking_errors.png", bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    initial_pos = [-45, -4, 0.8]  # Design initial position
    
    # Define different N values for testing
    N_values = [30]
    
    # Save results for different N values
    results = {}
    ...
    plt.savefig("adaptive_imgs/mpc_comparison_different_N.png", dpi=300)